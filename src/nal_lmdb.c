...
{
    return mdb_txn_begin(env.env, (MDB_txn *)parent, 0, txn);
}

int nal_readonly_txn_begin(nal_txn_ptr parent, nal_txn_ptr *txn)
{
    return mdb_txn_begin(env.env, (MDB_txn *)parent, MDB_RDONLY, txn);
}
...
{
    return mdb_put(txn, dbi, (MDB_val *)key, (MDB_val *)data, 0);
}

int nal_del(nal_txn_ptr txn, nal_dbi dbi, nal_val *key)
{
    return mdb_del(txn, dbi, (MDB_val *)key, (MDB_val *)NULL);
}

int nal_get(nal_txn_ptr txn, nal_dbi dbi, nal_val *key, nal_val *data)
{
    return mdb_get(txn, dbi, (MDB_val *)key, (MDB_val *)data);
}
...
int nal_cursor_get(nal_cursor_ptr cursor, nal_val *key, nal_val *data,
                   MDB_cursor_op op)
{
    return mdb_cursor_get(cursor, (MDB_val *)key, (MDB_val *)data, op);
}

int nal_cursor_put(nal_cursor_ptr cursor, nal_val *key, nal_val *data,
                   unsigned int flags)
{
    return mdb_cursor_put(cursor, (MDB_val *)key, (MDB_val *)data, flags);
}
